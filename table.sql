...
create index IF not exists idx_activity_logs_session_id on public.activity_logs using btree (session_id) TABLESPACE pg_default;

create index IF not exists idx_activity_logs_student_id on public.activity_logs using btree (student_id) TABLESPACE pg_default;

create index IF not exists idx_activity_logs_exam_id on public.activity_logs using btree (exam_id) TABLESPACE pg_default;
...
create index IF not exists idx_activity_logs_event_type on public.activity_logs using btree (event_type) TABLESPACE pg_default;
...
create index IF not exists idx_audit_logs_actor on public.audit_logs using btree (actor_type, actor_id) TABLESPACE pg_default;
...
create index IF not exists idx_exam_risk_logs_session_id on public.exam_risk_logs using btree (session_id) TABLESPACE pg_default;

create index IF not exists idx_exam_risk_logs_student_id on public.exam_risk_logs using btree (student_id) TABLESPACE pg_default;

create index IF not exists idx_exam_risk_logs_timestamp on public.exam_risk_logs using btree ("timestamp") TABLESPACE pg_default;
...
create table public.exam_sessions (
  ...
  last_tab_switch_at timestamp with time zone null,
...
create unique INDEX IF not exists unique_student_exam_active_session on public.exam_sessions using btree (student_id, exam_id) TABLESPACE pg_default
where
  (status = 'in_progress'::text);

create index IF not exists idx_exam_sessions_end_time on public.exam_sessions using btree (end_time) TABLESPACE pg_default;
...
create unique INDEX IF not exists grade_sections_grade_name_stream_key on public.grade_sections using btree (
  grade_id,
  section_name,
  COALESCE(stream, ''::text)
) TABLESPACE pg_default;
