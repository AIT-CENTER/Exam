...
create trigger mark_answer_trigger BEFORE INSERT
or
update on student_answers for EACH row when (new.selected_option_id is not null)
execute FUNCTION mark_answer ();

create trigger update_exam_activity_trigger
after INSERT
or
update on student_answers for EACH row
